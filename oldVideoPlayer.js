...
export default class VideoPlayer extends Component {
  ...
  _onLoad(data = {}) {
    ...
    //android rate reset bugfix (ugly patch)
    if (state.resetRate) {
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    const {source, paused, videoStyle, style} = nextProps;
    if (this.state.paused !== paused) {
      this.setState({paused});
    }

    if (this.styles.videoStyle !== videoStyle) {
      this.styles.videoStyle = videoStyle;
    }

    if (this.styles.containerStyle !== style) {
